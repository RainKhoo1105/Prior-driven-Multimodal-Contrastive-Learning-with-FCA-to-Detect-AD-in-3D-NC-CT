...
class DatasetLoader:
    def __init__(
        ...
        Params:
            ...
            match_path: csv path for diagnostic reports, support multiple inputs
            save_path: save path
            threshold: CT and CTA image truncation threshold
            save_verbose: whether to visualize the dataset
            train_ration: training set ratio
            resolution: default image resolution
            text_mode: text processing mode, currently supports two modes: 1. keep all diagnostic reports; 2. use jieba to segment keywords
            timestamp: dataset timestamp, if specified, load the dataset packed at that timestamp, otherwise load the latest dataset
